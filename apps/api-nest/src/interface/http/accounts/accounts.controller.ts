...
export class AccountsController {
    ...
    @Patch(":id")
    @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
    async rename(@Req() req: Request, @Param("id") accountId: string, @Body() body: RenameAccountDto) {
        const userId = await this.extractUserIdOrThrow(req);
        const result = await this.renameAccount.execute({ accountId, userId, name: body.name });
        if (!result.ok) {
            throw result.error;
        }
        return { account: result.value };

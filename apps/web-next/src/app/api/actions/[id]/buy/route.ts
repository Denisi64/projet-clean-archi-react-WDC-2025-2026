...
const buyActionUC = new BuyActionUseCase(actionRepo, portfolioRepo, tradeRepo, notifier);
...
async function requireClient(req: NextRequest): Promise<{ userId: string } | NextResponse> {
    ...
    if (!auth.ok) {
        ...
        if (isDev) console.error("[actions buy] auth error:", e?.message);
...
async function handleUseCase(req: NextRequest, actionId: string) {
    ...
    const result = await buyActionUC.execute({
        userId: auth.userId,
        actionId,
    ...
    if (!result.ok) {
        ...
        if (e instanceof ActionUnavailableError) {
            return NextResponse.json({ code: "ACTION_UNAVAILABLE" }, { status: 409 });
        }
        if (e instanceof InsufficientActionStockError) {
            return NextResponse.json({ code: "INSUFFICIENT_ACTION_STOCK" }, { status: 409 });
        }
        if (e instanceof InvalidActionQuantityError) {
            return NextResponse.json({ code: "INVALID_ACTION_QUANTITY" }, { status: 400 });
        }
        if (isDev) console.error("[actions buy] error:", e?.message);
        return NextResponse.json({ code: "UNEXPECTED_ERROR" }, { status: 500 });
    }
...
async function handleProxy(req: NextRequest, actionId: string) {
    ...
    const url = `${base}/actions/${actionId}/buy`;
    ...
    try {
        ...
        if (isDev) console.error("[actions buy] upstream error:", e?.message);

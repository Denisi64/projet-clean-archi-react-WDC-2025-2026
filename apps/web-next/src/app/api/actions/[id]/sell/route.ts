...
const sellActionUC = new SellActionUseCase(
  actionRepo,
  portfolioRepo,
  tradeRepo,
  notifier
...
async function requireClient(
  ...
  const auth = await getUserRoleUC.execute({
    token: session,
  ...
  if (!result.ok) {
    ...
    if (e instanceof InsufficientActionQuantityError) {
      return NextResponse.json(
        { code: "INSUFFICIENT_ACTION_QUANTITY" },
        { status: 409 }
      );
    }
    if (e instanceof InvalidActionQuantityError) {
      return NextResponse.json(
        { code: "INVALID_ACTION_QUANTITY" },
        { status: 400 }
      );
    }
...
export async function POST(
  ...
  return target === "next"
    ? handleUseCase(req, actionId)
    : handleProxy(req, actionId);
}
